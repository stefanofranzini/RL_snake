...
# TODO: change critter brain

def critter_brain(x2, y2, x1, y1, foodx, foody):

    delta_danger_x = x2 - x1
    delta_danger_y = y2 - y1
    delta_food_x = x2 - foodx
    delta_food_y = x2 - foody

    probs = np.zeros(5)

    if abs(delta_food_x) < abs(delta_danger_x):
        if delta_food_x < 0:
            probs[3] += 1  # dx
        elif delta_food_x > 0:
            probs[2] += 1  # sx
        ...
        if delta_danger_x < 0:
            probs[2] += 1  # sx
        elif delta_danger_x > 0:
            probs[3] += 1  # dx

    if abs(delta_food_y) < abs(delta_danger_y):
        if delta_food_y < 0:
            probs[1] += 1  # dw
        elif delta_food_y > 0:
            probs[0] += 1  # up
